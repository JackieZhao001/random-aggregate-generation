...
while Rat2<=0.3
    ...
    O2(k,3)=unifrnd(-temp_r,temp_r);
    O2(k,2)=unifrnd(4,temp_h);%球心坐标
    if Rat2<0.05
        R2(k)=unifrnd(9,10);%半径
    elseif (Rat2>0.05) && (Rat2<=0.1)
        R2(k)=unifrnd(8,9);
    elseif (Rat2>0.1) && (Rat2<=0.15)
        R2(k)=unifrnd(7,8);
    elseif (Rat2>0.15) && (Rat2<=0.2)
        R2(k)=unifrnd(6,7);
    elseif (Rat2>0.2) && (Rat2<=0.25)
        R2(k)=unifrnd(5,6);
    else
        R2(k)=unifrnd(4,5);
    end
%     R2(k)=(1+ScaR)*R2(k);           %(将骨料扩为界面)
    %判断球体是否在边界内
    if O2(k,2)<R2(k) || O2(k,2)>h0-R2(k) || sqrt(O2(k,1)^2+O2(k,3)^2)>r0-R2(k)
        continue;                             %返回重新循环
    end   
    %缩小生成范围
    for j=1:(k-1)
...
while Rat1<=0.06
    ...
    O1(kk,3)=unifrnd(-temp_r,temp_r);
    O1(kk,2)=unifrnd(2.5,temp_h);%球心坐标
    if Rat1<0.025
    ...
    %判断球体是否在边界内
    if O1(kk,2)<R1(kk) || O1(kk,2)>h0-R1(kk) || sqrt(O1(kk,1)^2+O1(kk,3)^2)>r0-R1(kk)
        continue;                             %返回重新循环
    end
    %缩小球生成范围------Po2
    for j=1:k-1
